    ...
    return (
        <div className={ `popup ${ props.isOpen ? "popup_opened" : "" }` } id="popup-profile">
            <div className="popup__container">
                ...
                <form className="form" name={ props.name } id="form-profile" noValidate>
                    <h2 className="form__header">{ props.title }</h2>
                    { props.children }
                    <button className="button button_low-transparent form__button" type="submit">{ props.titleButton }</button>
                </form>
            </div>
        </div>
    )
